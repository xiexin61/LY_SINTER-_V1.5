...
namespace LY_SINTER.Popover.Parameter
{
    ...
    public partial class Ingredient_supplement : Form
    {
        public System.Timers.Timer _Timer1 { get; set; }
        string L3_CODE = Raw_Popup.L3_CODE_BL;
        public static bool isopen = false;
        DBSQL dBSQL = new DBSQL(ConstParameters.strCon);
        int _flag = 0;
        public vLog _vLog { get; set; }
        public Ingredient_supplement()
        {
            InitializeComponent();
            if (_vLog == null)
                _vLog = new vLog(".\\log_Page\\Parametery\\Ingredient_Protect_Pop_supplement_ADD\\");
            classify();
            data_text();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //产地
                string CD = textBox6.Text.ToString() == ""? "NULL" : textBox6.Text.ToString();
                //单价
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            var _sql = "update M_MATERIAL_ANALYSIS set FLAG = 2 where L3_CODE = '" + L3_CODE + "' ";
                                            dBSQL.CommandExecuteNonQuery(_sql);

                                            //int _L2_CODE = int.Parse(L2_CODE.ToString());
                                            //if (_L2_CODE >= 120 && _L2_CODE <= 299)
                                            //{
                                            //    string sql_insert = "INSERT INTO M_MATERIAL_COOD (TIMESTAMP,L2_CODE,L3_CODE,MAT_DESC,M_CODE,UNIT_PRICE,PLACE_ORIGIN)VALUES('" + dateTime + "'," + L2_CODE + ",'" + L3_CODE + "','" + WLMS + "','" + LH + "','" + DJ + "','" + CD + "')";
                                            //    dBSQL.CommandExecuteNonQuery(sql_insert);
                                            //}
                                            //else
                                            //{
                                            //    string sql_insert = "INSERT INTO M_MATERIAL_COOD (TIMESTAMP,L2_CODE,L3_CODE,MAT_DESC,M_CODE)VALUES('" + dateTime + "'," + L2_CODE + ",'" + L3_CODE + "','" + WLMS + "','" + LH + "')";
                                            //    dBSQL.CommandExecuteNonQuery(sql_insert);
                                            //}
                                            for (int x = 0; x < dataGridView1.ColumnCount; x++)
                                            {
